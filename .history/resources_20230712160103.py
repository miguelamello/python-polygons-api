...
class ProviderResource(Resource):
    ...
    def put(self, provider_id):
        ...
        # Verify required fields
        # Exit if any of the them are missing or empty
        response = Utils.validateFields(data)
        if response != True:
            return response

        # Format payload to expected syntaxe
        data = Utils.formatPayload(data)
...
class ServiceAreaResource(Resource):
    ...
    def post(self):
        ...
        # Verify if provider entries already exists
        # Exit if any of the entries already exists
        if ServiceArea.objects(name=data.get('name')):
            return {'error': 'ServiceArea with the same name already exists'}, 400

        if ServiceArea.objects(email=data.get('price')):
            return {'error': 'ServiceArea with the same email already exists'}, 400

        if ServiceArea.objects(phone=data.get('phone')):
            return {'error': 'ServiceArea with the same phone already exists'}, 400
        ...
        # Create provider
        try:
            provider = ServiceArea(**data)
            provider.save()
            return {'message': 'ServiceArea created successfully', 'id': str(provider.id)}
        except ValidationError as e:
            return {'error': str(e)}, 400
        except Exception as e:
            return {'error': str(e)}, 404
