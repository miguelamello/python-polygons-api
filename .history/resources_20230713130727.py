...
class ServiceAreaResource(Resource):
    ...
    def post(self):
        ...
        if ServiceArea.objects(name=data['geopos']['coordinates']):
            return {'error': 'ServiceArea with the same Geoposition already exists'}, 400
    ...
    def put(self, service_area_id):
        ...
        try:
            provider = ServiceArea.objects.get(id=provider_id)
        except DoesNotExist:
            return {'error': 'Service Area ' + provider_id + ' does not exist'}, 404
        except Exception as e:
            return {'error': str(e)}, 404

        try:
            ServiceArea.objects(id=provider_id).update_one(**data)
            return {'message': 'Service Area ' + provider_id + ' updated successfully'}
        except ValidationError as e:
            return {'error': str(e)}, 400
        except Exception as e:
            return {'error': str(e)}, 404
    ...
    def delete(self, service_area_id):
        try:
            ...
            return {'error': 'ServiceArea ' + service_area_id + ' does not exist'}, 404
        except Exception as e:
            return {'error': str(e)}, 404

        try:
            ServiceArea.objects(id=service_area_id).delete()
            return {'message': 'ServiceArea ' + service_area_id + ' deleted successfully'}
        except Exception as e:
            return {'error': str(e)}, 404
